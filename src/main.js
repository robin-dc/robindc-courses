...
function renderCategoryBtns(){
  ...
  .map(btn => {
    return   `<button class="text-tertiary py-[0.5rem] px-1 font-semibold hover:text-white filter-btn" data-id="${btn}">${btn.toUpperCase()}</button>`
  })
  .join("")
...
function renderCourse(courseArray){
    const render = courseArray.map(course => {
        return `<div class="flex justify-center flex-col gap-1 pb-1 bg-bColor rounded-md overflow-hidden">
                    <div class="w-full relative h-9">
                        <iframe src="https://www.youtube.com/embed/${course.link}" frameborder="0" allowfullscreen  height="9rem" padding="0px" class=" lazyload z-[999] mt-0 pt-0 opacity-100 w-full absolute left-0 top-0 right-0 h-9"></iframe>
                    ...
                    <h4 class="text-white font-medium text-base px-1">${course.title}</h4>
                    <ul class="flex gap-[0.5rem] self-start px-1">
                        <li class="text-[#90c53f] px-[0.5rem] py-[0.3rem] rounded-lg bg-black font-medium flex items-center"> <small class="text-[0.7rem] md:text-[0.8rem]">#coding</small> </li>
                        <li class="text-[#00d8ff] px-[0.5rem] py-[0.3rem] rounded-lg bg-black font-medium flex items-center"> <small class="text-[0.7rem] md:text-[0.8rem]">#webdev</small> </li>
                        <li class="text-[#f0db4f] px-[0.5rem] py-[0.3rem] rounded-lg bg-black font-medium flex items-center"> <small class="text-[0.7rem] md:text-[0.8rem]">#bscpe</small> </li>
                    </ul>
                </div>`
    })
    .join("")
